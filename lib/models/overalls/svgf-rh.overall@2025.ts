...
export default {
  id: 'svgf-rh.overall@2025',
  name: 'SvGF Rikshoppet Overall 2025',
  options: [],
  ...
  rankOverall (results, options, competitionEventOptions) {
    ...
    const ranked = participantIds.map(participantId => {
      ...
      const PSum = cRes
        .filter(res => parseCompetitionEventDefinition(res.meta.competitionEvent).type === 'fs')
        .reduce((acc, curr) => acc + (curr.result.P ?? 0), 0)
    ...
    for (const result of ranked) {
      result.result.S = ranked.findIndex(obj =>
        obj.result.T === result.result.T &&
        obj.result.PSum === result.result.PSum
      ) + 1
    }
